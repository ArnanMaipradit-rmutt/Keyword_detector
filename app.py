...
def get_pdf_fonts():
    """
    Finds standard Unicode fonts for PyMuPDF that support both Thai and English.
    """
    candidates = [
        ("C:/Windows/Fonts/tahoma.ttf", "C:/Windows/Fonts/tahomabd.ttf", "tahoma", "tahomabd"),
        ("C:/Windows/Fonts/leelawad.ttf", "C:/Windows/Fonts/leelawdb.ttf", "leelawadee", "leelawadeebd"),
        ("C:/Windows/Fonts/arial.ttf", "C:/Windows/Fonts/arialbd.ttf", "arial", "arialbd"),
        ("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", "dejavu", "dejavubd")
    ]
    for reg, bd, reg_name, bd_name in candidates:
        if os.path.exists(reg):
            bd_path = bd if os.path.exists(bd) else reg
            return reg, bd_path, reg_name, bd_name
    return None, None, "helv", "helv"


def append_summary_pages(doc, pdf_name, summary_data, report_data, total_matches, total_pdf_words, kw_color_map):
    """
    Appends beautifully styled summary page(s) at the end of the PDF.
    Contains overview metadata, keyword breakdown with colored swatches, counts, and percentages.
    """
    font_reg, font_bd, fn_reg, fn_bd = get_pdf_fonts()
    ...
    page.insert_text((45, 50), 'สรุปผลการวิเคราะห์และไฮไลต์คำสำคัญ (Keyword Analytics Summary)', 
                     fontsize=12, fontname=fn_bd, fontfile=font_bd, color=(1, 1, 1))
    page.insert_text((45, 70), 'KeyPDF Highlight & Analytics Studio | เอกสารสรุปผลการตรวจจับอัตโนมัติ', 
                     fontsize=8.5, fontname=fn_reg, fontfile=font_reg, color=(0.8, 0.85, 0.9))
    ...
    page.insert_text((45, 115), 'ชื่อเอกสาร:', fontsize=9.5, fontname=fn_bd, fontfile=font_bd, color=(0.2, 0.25, 0.3))
    page.insert_text((105, 115), str(pdf_name)[:55], fontsize=9.5, fontname=fn_reg, fontfile=font_reg, color=(0.1, 0.1, 0.1))
    page.insert_text((430, 115), f'จำนวนหน้าเดิม: {original_pages} หน้า', fontsize=9, fontname=fn_reg, fontfile=font_reg, color=(0.3, 0.35, 0.4))
    
    # Line 2: Metrics
    overall_density = (total_matches / total_pdf_words * 100) if total_pdf_words > 0 else 0.0
    page.insert_text((45, 140), f'คำทั้งหมดใน PDF: {total_pdf_words:,} คำ', fontsize=9, fontname=fn_reg, fontfile=font_reg, color=(0.25, 0.3, 0.35))
    page.insert_text((220, 140), f'คำสำคัญที่พบ: {total_matches:,} จุด', fontsize=9, fontname=fn_bd, fontfile=font_bd, color=(0.05, 0.55, 0.35))
    page.insert_text((380, 140), f'สัดส่วนคำสำคัญ: {overall_density:.4f}%', fontsize=9, fontname=fn_reg, fontfile=font_reg, color=(0.25, 0.3, 0.35))
    
    # 3. Section Title
    page.insert_text((30, 173), '📊 ตารางแจกแจงสถิติคำสำคัญและสีไฮไลต์ (Keyword Breakdown)', 
                     fontsize=10.5, fontname=fn_bd, fontfile=font_bd, color=(0.1, 0.15, 0.25))
    ...
    for title, x in cols:
        page.insert_text((x, y_table + 15), title, fontsize=8, fontname=fn_bd, fontfile=font_bd, color=(1, 1, 1))
    ...
    for idx, item in enumerate(summary_data):
        ...
        if idx > 0 and idx % 26 == 0:
            page.insert_text((45, 820), f'หน้าสรุปผลการวิเคราะห์ท้ายเอกสาร | {pdf_name}', fontsize=8, fontname=fn_reg, fontfile=font_reg, color=(0.5, 0.5, 0.6))
            page = doc.new_page(width=595, height=842)
            cur_y = 40
            page.draw_rect(fitz.Rect(30, cur_y, 565, cur_y + 22), color=None, fill=(0.12, 0.16, 0.23))
            for title, x in cols:
                page.insert_text((x, cur_y + 15), title, fontsize=8, fontname=fn_bd, fontfile=font_bd, color=(1, 1, 1))
            cur_y += 22
        ...
        page.insert_text((42, cur_y + 14), str(idx + 1), fontsize=8, fontname=fn_reg, fontfile=font_reg, color=(0.4, 0.4, 0.4))
        page.insert_text((65, cur_y + 14), str(kw)[:26], fontsize=8.5, fontname=fn_bd, fontfile=font_bd, color=(0.1, 0.1, 0.1))
        
        # Draw Color Swatch Box
        page.draw_rect(fitz.Rect(215, cur_y + 4, 255, cur_y + 16), color=(0.6, 0.65, 0.7), fill=rgb, width=0.5)
        
        page.insert_text((270, cur_y + 14), str(color_name)[:18], fontsize=8, fontname=fn_reg, fontfile=font_reg, color=(0.25, 0.25, 0.25))
        page.insert_text((385, cur_y + 14), str(cnt), fontsize=8.5, fontname=fn_bd, fontfile=font_bd, color=(0.05, 0.45, 0.25))
        page.insert_text((440, cur_y + 14), str(p_pdf), fontsize=8, fontname=fn_reg, fontfile=font_reg, color=(0.3, 0.3, 0.3))
        page.insert_text((500, cur_y + 14), str(p_kw), fontsize=8, fontname=fn_reg, fontfile=font_reg, color=(0.3, 0.3, 0.3))
        
        cur_y += row_h

    # Footer
    page.insert_text((45, 820), f'หน้าสรุปผลการวิเคราะห์ท้ายเอกสาร | KeyPDF Highlight & Analytics Studio | {pdf_name}', 
                     fontsize=8, fontname=fn_reg, fontfile=font_reg, color=(0.5, 0.5, 0.6))
...
def generate_batch_zip(all_results):
    """
    Packages all highlighted PDFs (with attached summary pages) and CSV files into a single ZIP archive.
    """
    zip_buf = io.BytesIO()
    with zipfile.ZipFile(zip_buf, "w", zipfile.ZIP_DEFLATED) as zf:
        for res in all_results:
            ...
            # Write CSV
            csv_bytes = generate_csv_bytes(res["summary"], res["report_data"], res["total_pdf_words"], res["total_matches"])
            zf.writestr(f"{pname}_keyword_report.csv", csv_bytes)
...
def main():
    ...
    uploaded_pdfs = st.sidebar.file_uploader(
        ...
        help="สามารถเลือกหรือลากวางไฟล์ PDF ได้ทีละหลายไฟล์พร้อมกัน"
    )
    ...
    with col1:
        ...
        if "all_results" in st.session_state and st.session_state["all_results"]:
            ...
            with d_col2:
                st.download_button(
                    label="📦 ดาวน์โหลดไฟล์ทั้งหมดรวม (.ZIP)",
                    data=st.session_state["batch_zip_bytes"],
                    file_name="All_Highlighted_PDFs_and_Reports.zip",
                    mime="application/zip",
                    help="ไฟล์ ZIP รวม PDF ที่ไฮไลต์พร้อมหน้าสรุปท้ายเล่ม และ CSV ทุกไฟล์",
                    use_container_width=True
                )
            ...
            with ind_col3:
                single_csv = generate_csv_bytes(cur_res["summary"], cur_res["report_data"], cur_res["total_pdf_words"], cur_res["total_matches"])
                st.download_button(
                    label="📑 โหลด CSV เฉพาะไฟล์นี้",
                    data=single_csv,
                    file_name=f"{pname_base}_keyword_report.csv",
                    mime="text/csv",
                    use_container_width=True
                )
